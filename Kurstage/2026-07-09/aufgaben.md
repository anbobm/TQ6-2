...
Wir wollen durchsetzen, dass die `personalnummer` des `Mitarbeiter`s in einem `Unternehmen` eindeutig ist. Dazu erstellen wir eine `MitarbeiterErzeugen()` Methode in `Unternehmen`, der einen `Mitarbeiter` nur erstellt, wenn es die übergebene `personalnummer` im Unternehmen nicht gibt.